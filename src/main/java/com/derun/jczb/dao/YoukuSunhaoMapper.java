...
public interface YoukuSunhaoMapper {
	...
	@Select("<script> select danjuhao from jiangsu.youkusunhao"
			+ " where niandu>=#{niandu} "
			+ " <if test='\"all\"!=youku'> "
			+ " and youku=#{youku} </if> "
			+ "  group by danjuhao order by danjuhao desc </script>")
	public List<String> queryBy(String youku,String niandu);
	@Insert("insert into jiangsu.youkusunhao(danjuhao,youku,riqi,oil,youpin,remark,niandu) values(#{danjuhao},#{youku},#{riqi},#{oil},#{youpin},#{remark},#{niandu})")
