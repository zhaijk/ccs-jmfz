...
public interface DiaobodanMapper {
	...
	@Select("select distinct danjuhao from diaobodan where niandu>=#{niandu} and leixing =1 order by danjuhao desc")
	public List<String> queryDanjuhao(String niandu);
	@Select("select * from Diaobodan where leixing in (1,2,7,8) and niandu > #{year} order by danjuhao desc")
	public List<Diaobodan> queryBy(String danjuhao,String year);
	@Select("select * from Diaobodan where rownum=1 and leixing =1 and niandu > #{year} order by danjuhao desc")
	public Diaobodan queryBySJ(String danjuhao,String year);
	@Select("<script> select a.*,(select junqu from JUNQU_DICTIONARY t where junqu_code=a.junqu_code) as danwei_name,(select youku from YOUKU_DICTIONARY t where youku_code=a.gongyingyouku) as youku_name from Diaobodan a "+
	...
	@Select("<script> select a.*,(select bumen from depar_dictionary  where bumen_code=a.shougongdanwei) as danwei_name,(select youku from YOUKU_DICTIONARY t where youku_code=a.gongyingyouku) as youku_name from Diaobodan a "+
	...
	@Select("<script> select a.*,(case leixing when 6 then (select bumen from depar_dictionary  where bumen_code=a.shougongdanwei)  else  (select youku from youku_dictionary  where youku_code=a.shougongdanwei)  end) as danwei_name,(select youku from YOUKU_DICTIONARY t where youku_code=a.gongyingyouku) as youku_name from Diaobodan a "+
	...
	@Select("select * from diaobodan_record  where fk_id=#{fk_id}")
	public List<DiaobodanRecord> queryByRecord(int fk_id);
	@Insert("insert into diaobodan (id,danjuhao,huandanhao,leixing,kaidanriqi,gongyingyouku,shougongdanwei,junqu_code,xiaoji,beizhu,niandu,biaozhi,caozuoyuan,caozuoriqi,caozuotime,beizhu_sys,dayin) values(#{id},#{danjuhao},#{huandanhao},#{leixing},#{kaidanriqi},#{gongyingyouku},#{shougongdanwei},#{junqu_code},#{xiaoji},#{beizhu},#{niandu},#{biaozhi},#{caozuoyuan},#{caozuoriqi},#{caozuotime},#{beizhu_sys},#{dayin})")
	public int insertOne(Diaobodan obj);
	@Select("select diaobodan_sequences.nextval from dual")
	public int queryDiaobodanId();
	@Select("select * from (select danjuhao from diaobodan where gongyingyouku=#{gongyingyouku} and (leixing =4 or leixing =5 or leixing=6) order by danjuhao desc) where rownum=1")
	public String queryMaxDanjuhao(String gongyingyouku);
	@Select("<script> select a.*,(select bumen from depar_dictionary  where bumen_code=a.shougongdanwei) as danwei_name,(select youku from YOUKU_DICTIONARY t where youku_code=a.gongyingyouku) as youku_name from diaobodan@remote a "
			+ "<where> a.leixing=2 and a.niandu=#{niandu} "
			+ "<if test='\"all\"!=gongyingyouku'> "
			+ "and gongyingyouku=#{gongyingyouku} </if> "
			+ "</where> order by kaidanriqi desc </script>")
	public List<Diaobodan> queryZG(String gongyingyouku,String niandu);
}
